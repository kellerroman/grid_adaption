...
contains
   ...
   !< 2: WURDE BEARBEITET
      ...
      ALLOCATE( UNSTR%PKT_DN     (UNSTR%NPKT,2))
      ALLOCATE( UNSTR%PKT_SOLL   (UNSTR%NPKT,2))
      ALLOCATE( UNSTR%CELL_INC_STRESS(UNSTR%NKNT))

      ALLOCATE( UNSTR%KNT        (UNSTR%NKNT,2))
      ALLOCATE( UNSTR%KNT_DN     (UNSTR%NKNT,4))
      ALLOCATE( UNSTR%KNT_SPANNUNG     (UNSTR%NKNT,2))

      UNSTR%PKT_TYPE = -1
      UNSTR%PKT_NKNT = 0
      UNSTR%PKT_KNT  = -1
      UNSTR%PKT_SOLL = 0.0D0
      UNSTR%CELL_INC_STRESS = 1.0D-10
